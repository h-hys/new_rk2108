...
static wiced_result_t brcm_dac_configure(void *driver_data, wiced_audio_config_t *config, uint32_t *mclk)
{
    ...
    {
    	#if 1
        abuf.period_size = 256;
        abuf.buf_size = abuf.period_size * 8;
        size = abuf.buf_size * 6 * (16 >> 3); /* frames to bytes */
        frame_size = abuf.period_size * 6 * (16 >> 3);

        abuf.buf = rt_malloc_uncache(size);
        RT_ASSERT(abuf.buf != NULL);

        param.channels = 6;//config->channels;
...
static wiced_result_t rk_recorder_configure(void *driver_data, wiced_audio_config_t *config, uint32_t *mclk)
{
    ...
    if (r_audio_device != NULL)
    {
        ...
        r_param.channels = 2;//config->channels;
        r_param.sampleRate = 16000;//config->sample_rate;
        r_param.sampleBits = 16;//config->bits_per_sample;
		#endif
