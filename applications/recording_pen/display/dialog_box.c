    ...
    if (type == TYPE_KEY_PRESS)
    {
        ...
        case KEY_UP:
            if (g_value == DIALOG_BOX_CONFIRM)
            {
                ...
                lv_obj_set_style(label_no, &label_style_font20_sel);
                lv_label_set_body_draw(label_no, 1);
                lv_obj_set_style(label_yes, &label_style_font20);
                lv_label_set_body_draw(label_yes, 0);
            }
            else
            {
                g_value = DIALOG_BOX_CONFIRM;
                lv_obj_set_style(label_yes, &label_style_font20_sel);
                lv_label_set_body_draw(label_yes, 1);
                lv_obj_set_style(label_no, &label_style_font20);
                lv_label_set_body_draw(label_no, 0);
            }
            break;
        case KEY_CANCEL:
...
void dialog_box_init(const char *str, dialog_callback cb)
{
    ...
    if (!dialog_box && !g_callback)
    {
        ...
        lv_obj_set_style(dialog_box, &label_style_font20);
        ...
        lv_label_set_recolor(dialog_box_text, true);
