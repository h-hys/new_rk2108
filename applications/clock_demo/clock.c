...
static rt_err_t app_clock_lvgl_clock_design(struct lvgl_clock_data *lv_clock_data)
{
    ...
    {
        /* date obj(label) init */
        snprintf(lv_clock_data->date_str, sizeof(lv_clock_data->date_str), "%02u月%02u日  星期%s",
        ...
        lv_obj_set_style(lv_clock_data->lv_date, &label_style);
 ...
 * lvgl touch callback.
 */
void lv_touch_cb(struct rt_touch_data *tp_data, rt_uint8_t num)
{
    struct rt_device_graphic_info *info = &g_lvgl_data->disp->info;

    rt_int16_t xoff  = LVGL_REGION_X + ((info->width   - LVGL_FB_W) / 2);
    rt_int16_t yoff  = LVGL_REGION_Y + ((LVGL_REGION_H - LVGL_FB_H) / 2);

    for (rt_uint8_t i = 0; i < 1/* num */; i++)
    {
        rt_int16_t x = tp_data[i].x_coordinate - xoff;
        rt_int16_t y = tp_data[i].y_coordinate - yoff;

        if (((0 < x) && (x < LVGL_FB_W)) && ((0 < y) && (y < LVGL_FB_H)))
        {
            if (tp_data[i].event == RT_TOUCH_EVENT_DOWN)
            {
                littlevgl2rtt_send_input_event(x, y, LITTLEVGL2RTT_INPUT_DOWN);
            }
            else if (tp_data[i].event == RT_TOUCH_EVENT_MOVE)
            {
                littlevgl2rtt_send_input_event(x, y, LITTLEVGL2RTT_INPUT_MOVE);
            }
            else if (tp_data[i].event == RT_TOUCH_EVENT_UP)
            {
                littlevgl2rtt_send_input_event(x, y, LITTLEVGL2RTT_INPUT_UP);
            }
...
static rt_err_t app_clock_lvgl_init(struct lvgl_clock_data *lv_clock_data)
{
...

    {
        ...
        rt_touchpad_thread_init("gtxx", lv_touch_cb);
#endif
