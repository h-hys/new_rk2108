...
#define CMD_MOVE_RUN                (0x01UL << 2)
#define CMD_MOVE_END                (0x01UL << 3)
...
{
    { 320, 320, "/clock_whiskey/clock_bkg_320_320_1.dta"},
    { 320, 320, "/clock_whiskey/clock_bkg_320_320_2.dta"},
    { 320, 320, "/clock_whiskey/clock_bkg_320_320_3.dta"},
};
...
// test: test timer
static void app_clock_test_timer(void *parameter)
{
    struct clock_whiskey_data *pdata = (struct clock_whiskey_data *)parameter;
...
static rt_err_t app_clock_whiskey_init(struct clock_whiskey_data *pdata)
{
    ...
    // test: run test demo
    pdata->test_timer = rt_timer_create("apptst_timer",
                                        app_clock_test_timer, (void *)pdata,
                                        APPCLK_CLOCK_TEST_TICKS + RT_TICK_PER_SECOND / 2,
                                        RT_TIMER_FLAG_ONE_SHOT);
    RT_ASSERT(pdata->test_timer != RT_NULL);
    rt_timer_start(pdata->test_timer);
...
static void app_clock_whiskey_deinit(struct clock_whiskey_data *pdata)
{
    ...
    rt_timer_stop(pdata->test_timer);
    rt_timer_delete(pdata->test_timer);
...
static rt_err_t app_clock_top_move_run(struct clock_whiskey_data *pdata)
{
    RT_ASSERT(ABS(pdata->topmov_dir) == 1);
#ifdef CLOCK_MOVE_UPDOWN
    RT_ASSERT(ABS(pdata->topmov_offset) < CLOCK_TOP_YRES);
#else
    RT_ASSERT(ABS(pdata->topmov_offset) < CLOCK_TOP_XRES);
#endif
...
static rt_err_t app_clock_bottom_move_run(struct clock_whiskey_data *pdata)
{
    RT_ASSERT(ABS(pdata->bottomov_dir) == 1);
#ifdef CLOCK_MOVE_UPDOWN
    RT_ASSERT(ABS(pdata->bottomov_offset) < CLOCK_BOTTOM_YRES);
#else
    RT_ASSERT(ABS(pdata->bottomov_offset) < CLOCK_BOTTOM_XRES);
#endif
...
static rt_err_t app_clock_refr_tasks(struct clock_whiskey_data *pdata)
{
    ...
    if ((pdata->cmd & CMD_MOVE_START) == CMD_MOVE_START)
    {
        ...
        pdata->cmd |= CMD_MOVE_RUN;
    }

    if ((pdata->cmd & CMD_MOVE_RUN) == CMD_MOVE_RUN)
    {
        //pdata->cmd &= ~CMD_MOVE_RUN;

        ret  = app_clock_top_move_run(pdata);
        ret1 = app_clock_bottom_move_run(pdata);
    ...
    if ((pdata->cmd & CMD_MOVE_END) == CMD_MOVE_END)
    {
        ...
        // test: continue test demo
        rt_tick_t arg = APPCLK_CLOCK_TEST_TICKS;
        rt_timer_control(pdata->test_timer, RT_TIMER_CTRL_SET_TIME, &arg);
        rt_timer_start(pdata->test_timer);
    }
