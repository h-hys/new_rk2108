...
static uint16_t vol_l = 24, vol_r = 24;
...
static bool s_record_flag = false;
...
struct TIMER_REG *pTimer2 = TIMER1;
extern void pmic_power_off(void);
static int power_down_flag = 0;
volatile static int timer_flag = 0;
...
void timer1_IRQHandler(void)
{
    ...
    pTimer2->INTSTATUS = 0x01;
	//rt_kprintf("suspend count: %d\n", rt_tick_get());
	
	app_led_control(LED_BT_PAIR, LED_NEGAT);
...
static void timer_wakeup()
{
    ...
    sec_to_count = 120 * 1000 * 1000 * 24;

	pTimer1->INTSTATUS = 0x00;
	pTimer1->CONTROLREG = 0x00;
    pTimer1->LOAD_COUNT[0] = 0x0EEBB000;
    pTimer1->LOAD_COUNT[1] = 0xA;
...
static void timer1_init()
{
    ...
    sec_to_count = 200 * 1000 * 24;

	pTimer2->INTSTATUS = 0x00;
	pTimer2->CONTROLREG = 0x00;
    pTimer2->LOAD_COUNT[0] = sec_to_count;
    pTimer2->LOAD_COUNT[1] = 0x0;
    pTimer2->CONTROLREG = 0x05;
	
    HAL_NVIC_SetIRQHandler(TIMER1_IRQn, (NVIC_IRQHandler) & timer1_IRQHandler);
...
static void app_thread_entry2(void *parameter)
{
	...
	while(1)
	{
		...
		if(!bt_connect_status())
		{
			...
			//五分钟内没有连接上蓝牙，切断电源
			if(end_tick - start_tick > 1000 * 300)
			{
				HAL_NVIC_DisableIRQ(TIMER1_IRQn);
				app_led_control(LED_BT_PAIR, LED_CLOSE);
				rt_kprintf("not connect bt device!, %d\n", rt_tick_get());
		...
		{			
			...
			//连接上蓝牙，30分钟没有任何操作，休眠；一小时没有操作，切断电源		
			if(!(bt_a2dp_play_status() || bt_get_hfp_status()))			
			{				
				if(end_tick - start_tick > 1000 * 1800)
				{					
