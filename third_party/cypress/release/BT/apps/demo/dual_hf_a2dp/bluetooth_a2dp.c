...
#define BT_ADDR "A4:9B:4F:F4:6A:E7"

void conn()
{
	wiced_bt_hfp_hf_connect(BT_ADDR);
	wiced_bt_a2dp_sink_connect(BT_ADDR);
}
MSH_CMD_EXPORT(conn, capture wav file);


void disconn()
{
	wiced_bt_hfp_hf_disconnect(BT_ADDR);
	wiced_bt_a2dp_sink_disconnect(BT_ADDR);
}
MSH_CMD_EXPORT(disconn, capture wav file);
...
{
	//rt_kprintf("0 is %d\n", a2dp_context[0].sink_connected);
	//rt_kprintf("1 is %d\n", a2dp_context[1].sink_connected);
	return (a2dp_context[0].sink_connected || a2dp_context[1].sink_connected);
}
