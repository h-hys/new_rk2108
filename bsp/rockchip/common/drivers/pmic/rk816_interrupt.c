...
static void rk816_isr_thread(void *p)
{
	...
	if (device == RT_NULL)
	{
		rt_kprintf("rt_device_find charger fail!\n");
		return NULL;
	}

	if(pmic_read(bus, addr, VB_MON_REG) & 0x40)
	{
		...
		rt_device_control(device, RT_DEVICE_CTRL_CHAGER_SRCCUR_SET, &data);
	}
    ...
    while (1)
    {
		...
		rt_kprintf("rk816 interrupt happen!\n");

		if(rt_current_pm_state() != 2)
		{
			rt_kprintf("222\n");
			rt_pm_request(1);
			
			//HAL_NVIC_SetPendingIRQ(TIMER0_IRQn);
		...
		if(pmic_read(bus, addr, INT_STS_REG3) == 1) //插入usb线
		{
			rt_device_control(device, RT_DEVICE_CTRL_CHAGER_SRCCUR_SET, &data);
			HAL_GPIO_SetPinLevel(GPIO1, GPIO_PIN_A7, 1);			
		}
		else if(pmic_read(bus, addr, INT_STS_REG3) == 2) //拔出usb线
		{
