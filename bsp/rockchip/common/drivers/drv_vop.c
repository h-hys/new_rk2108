...
static void rockchip_vop_enable(struct display_state *state)
{
    ...
    if (state->panel_state.display_mode == DISPLAY_CMD_MODE)
        HAL_VOP_EdpiInit(crtc_state->hw_base);
...
static void rockchip_vop_disable(struct display_state *state)
{
    ...
    while (!HAL_VOP_CommitPrepare(crtc_state->hw_base))
    {
        if (i++ > HOLD_MODE_TIME_OUT)
        {
            rt_kprintf("rockchip_vop_disable wait hold mode time out\n");
            break;
        }
        rt_thread_mdelay(1);
    }
...
static int rockchip_display_sync(struct display_state *state)
{
    ...
    return HAL_VOP_CommitPrepare(crtc_state->hw_base);
}
